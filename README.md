...
ValuAgent is optimized for deployment on Render.com, which offers a free tier with native Docker support and no credit card required.
...
8. You receive a live URL like `https://valuagent.onrender.com`.
...
gcloud run deploy valuagent --image gcr.io/YOUR_PROJECT_ID/valuagent --platform managed --region us-central1 --allow-unauthenticated --timeout 300s --memory 1Gi --set-env-vars="GROQ_API_KEY=your_key"
```

## Architecture
ValuAgent uses a sequential runner managed by the Google ADK. Each agent passes its output via session state variables. The critic_agent operates as a strict financial guardrail. The LLM is restricted to providing purely factual, backward-looking synthesis.
